...
const Skiggle = () => {
  ...
  // The line is drawn over the actual about-section viewport range instead of
  // a fixed fraction of the whole document, so its timing remains correct
  // after the profile/education content changes the page height.
  const dashOffset = useTransform(scrollYProgress, [0, 1], [-4291, 0]);
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener("resize", setPathLength);
  }, []);
  ...
  return (
    ...
    >
      <svg
        className="squigggle absolute left-1/2 top-[-10%] h-full w-[120vw] -translate-x-1/2"
        fill="none"
